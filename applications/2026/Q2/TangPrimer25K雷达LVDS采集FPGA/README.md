...
- 淘宝记录: 2026-05-21 淘宝桌面版短搜 `Tang Primer 25K`，Sipeed 矽速科技官方店搜索结果价 `¥49.00`。
...
- 淘宝价格截图: ![Tang Primer 25K 淘宝价格截图](assets/taobao-tang-primer-25k-price.png)
...
本采购与自研 1Gbps USB3.0 雷达采集卡形成互补：Tang Primer 25K 用于快速验证 LVDS 接收和 FPGA 逻辑原型，自研 USB3.0 采集卡用于将稳定后的采集链路固化到高速上位机接口。通过先在开发板上完成时序约束、接口映射、测试向量和缓存策略验证，可以降低自研板调试风险，减少 PCB 回板后定位问题的时间成本。

### 资料提取结论

| 资料项 | 访问结果 | 对申报的作用 |
| --- | --- | --- |
| 淘宝搜索页 | 短搜 `Tang Primer 25K` 可定位 Sipeed 矽速科技官方店，搜索结果价 `¥49.00` | 支撑预算金额和采购对象 |
...
| --- | ---: | ---: | ---: | --- |
| Tang Primer 25K FPGA 开发套件 | 1 | 49.00 | 49.00 | Sipeed 矽速科技官方店搜索结果价；核心板 + 25K Dock 套件 |
| 合计 |  |  | 49.00 | 本表为淘宝桌面版当前记录价；实际支出以下单页和发票/订单为准 |

## 采购理由

- 用于雷达 LVDS 原始数据采集前端逻辑验证，降低自研采集卡直接调试风险。
...
- 淘宝价格截图、商品截图、订单截图和到货照片补充到 `assets/`。
