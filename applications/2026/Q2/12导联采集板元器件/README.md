...
- 商品名称: 立创商城 12导联采集板核心元器件购物车
- 申报名称: 12导联心电采集板元器件采购
- 选定规格: ADS1298R AFE、T113-S3、1G NAND、ESP32、连接器、电源、保护与阻容件等 76 项
- 主要用途: 用于 12 导联心电采集板样机焊接、调试和功能验证。
- 资料来源: 本地下载文件 `立创商城购物车详情 (1).xls`，解析得到 76 项，合计 792.70 元。
...
本项目拟采购 12 导联心电采集板样机所需核心元器件，覆盖生物电模拟前端、主控/通信、存储、电源管理、保护器件、连接器、晶振、咪头和基础阻容件。清单中的 ADS1298R 是 8 通道 24 bit 生物电测量模拟前端，是多导联心电采集链路的关键器件；T113-S3 可用于板端控制、数据缓存与接口调度；ESP32、NAND Flash、Type-C、PicoBlade/FPC 连接器和电源/保护器件共同支撑样机从采集、控制、供电到数据传输的完整闭环。
...
| 合计 |  |  | 792.70 | 当前价格依据本地下载 xls 或淘宝桌面版截图记录，实际支出以下单页和发票/订单为准 |
...
2. 按器件类别分装，优先核对 ADS1298R、T113-S3、NAND Flash、电源芯片和连接器。
