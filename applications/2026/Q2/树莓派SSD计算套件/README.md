...
- 资料来源: 商品页面价格记录：PCIe M.2 SSD 套餐、Pi5/16GB 规格，店铺优惠后 ¥2969。
...
16GB 内存和 PCIe M.2 SSD 是本项目的关键配置。16GB 内存可为雷达采集进程、心音心电采集进程、数据队列、质量检查脚本、文件打包任务和远程管理服务同时运行预留余量，降低多进程并发时丢帧、阻塞和异常退出的风险。PCIe M.2 SSD 则用于承担连续采集的本地高速缓存，避免 SD 卡在高频写入、长时间实验和反复擦写场景下的速度衰减与损耗。主动散热和稳定 PD 供电可以减少长时间采集中的降频和电源不稳，使该终端更适合随实验场景移动部署。

### 资料提取结论

| 资料项 | 访问结果 | 对申报的作用 |
| --- | --- | --- |
| 商品页面记录 | 选中 PCIe M.2 SSD 套餐、Pi5/16GB，优惠后 ¥2969 | 支撑价格记录 |
